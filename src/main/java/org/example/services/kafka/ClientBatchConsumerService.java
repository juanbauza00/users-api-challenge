...
public class ClientBatchConsumerService {
    ...
    /* NOTA 1* (Colocada en @KafkaListener)
    ...
    NOTA 2* - Parámetros del método consumeClientBatch:
    ...
    */
    @KafkaListener(//NOTA 1*
            topics = "${kafka.topics.client-batch}",
            groupId = "${kafka.consumer.group-id}",
            containerFactory = "kafkaListenerContainerFactory"
    )
    public void consumeClientBatch(//NOTA 2*
            @Payload ClientBatch clientBatch,
            @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
            @Header(KafkaHeaders.RECEIVED_PARTITION_ID) int partition,
            @Header(KafkaHeaders.OFFSET) long offset,
        ...
        try{
            ...
            acknowledgment.acknowledge(); //NOTA 3*
    ...
    NOTA 3* - Acknowledgment (acknowledgment.acknowledge()):
    ...
    NOTA 4* - Validaciones en Consumer:
